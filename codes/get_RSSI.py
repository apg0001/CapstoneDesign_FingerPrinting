...
# 📌 데이터 변환
X_list, y_list = [], []
max_ap = 20  # 한 번의 측정에서 최대 20개 AP 사용
...
for epoch in epoch_progress:
    ...
    # 🟢 tqdm으로 전체 Epoch 진행률 업데이트
    epoch_progress.set_postfix(loss=total_loss / len(train_loader), accuracy=f"{accuracy:.2f}%")

    # ✅ Epoch별 로그 출력
    # print(f"✅ Epoch [{epoch+1}/{num_epochs}] - Predicted : {predicted}, y_batch : {y_batch}, Loss: {total_loss/len(train_loader):.4f}, Accuracy: {accuracy:.2f}%")
